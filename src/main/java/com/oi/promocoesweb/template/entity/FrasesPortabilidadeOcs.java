 ...
 * @author Jpereirc
 */
public class FrasesPortabilidadeOcs extends Template {
    ...
    public FrasesPortabilidadeOcs() {
        super();
    }

    public FrasesPortabilidadeOcs(FrasesPortabilidadeOcs selectedConfigOfertasPre) {
    ...
    public String getIdDdd() {
        ...
        if (ddd != null && ddd.length() >= 2) {
            return sb.append("IDOFERTAOCS: ").append(idofertaocs).append(" | IDBENEFICIO: ").append(idbeneficio).append(" | IDPROGRAMA: ").append(idprograma).append(" | DDD: ").append(ddd.substring(0, 2)).append("...").toString();
        } else {
            return sb.append("IDOFERTAOCS: ").append(idofertaocs).append(" | IDBENEFICIO: ").append(idbeneficio).append(" | IDPROGRAMA: ").append(idprograma).append(" | DDD: null").toString();
        }
    }
    ...
    public void setHistory(String history) throws Exception {
        if (history != null && !"".equals(history)) {
            try {
                ...
                FrasesPortabilidadeOcs[] pp1 = mapper.readValue(history, FrasesPortabilidadeOcs[].class);
                setHistory(Arrays.asList(mapper.readValue(history, FrasesPortabilidadeOcs[].class)));

                Date tempDate = new Date(0);
                User tempUser = null;
                for (FrasesPortabilidadeOcs template : pp1) {
