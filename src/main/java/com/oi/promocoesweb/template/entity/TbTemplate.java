...
public class TbTemplate extends Template{
    ...
    public TbTemplate(TbTemplate selectedConfigOfertasPre){
    ...
    @JsonIgnore public boolean isEditOferta1() {return super.getEditMap().getOrDefault("OFERTA1", false);}
    @JsonIgnore public boolean isEditOferta2() {return super.getEditMap().getOrDefault("OFERTA2", false);}
    @JsonIgnore public boolean isEditOferta3() {return super.getEditMap().getOrDefault("OFERTA3", false);}
    @JsonIgnore public boolean isEditOferta_erro_in() {return super.getEditMap().getOrDefault("OFERTA_ERRO_IN", false);}
    @JsonIgnore public boolean isEditOferta_erro_siebel() {return super.getEditMap().getOrDefault("OFERTA_ERRO_SIEBEL", false);}
    @JsonIgnore public boolean isEditSegprompt() {return super.getEditMap().getOrDefault("SEGPROMPT", false);}
    ...
    public void setEditOferta1(boolean checked) {super.getEditMap().put("OFERTA1", checked);}
    public void setEditOferta2(boolean checked) {super.getEditMap().put("OFERTA2", checked);}
    public void setEditOferta3(boolean checked) {super.getEditMap().put("OFERTA3", checked);}
    public void setEditOferta_erro_in(boolean checked) {super.getEditMap().put("OFERTA_ERRO_IN", checked);}
    public void setEditOferta_erro_siebel(boolean checked) {super.getEditMap().put("OFERTA_ERRO_SIEBEL", checked);}
    public void setEditSegprompt(boolean checked) {super.getEditMap().put("SEGPROMPT", checked);}
    ...
    public void setHistory(String history) throws Exception {
        if (history != null && !"".equals(history)) {
            try {
                ...
                TbTemplate[] pp1 = mapper.readValue(history, TbTemplate[].class);
                setHistory(Arrays.asList(mapper.readValue(history, TbTemplate[].class)));

                Date tempDate = new Date(0);
                User tempUser = null;
                for (TbTemplate template : pp1) {
