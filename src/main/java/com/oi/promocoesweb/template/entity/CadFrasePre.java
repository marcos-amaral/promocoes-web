 ...
 * @author mmouraam
 */
public class CadFrasePre extends Template {
    ...
    public CadFrasePre(CadFrasePre selectedConfigOfertasPre) {
    ...
    public void setHistory(String history) throws Exception {
        if (history != null && !"".equals(history)) {
            try {
                ...
                CadFrasePre[] pp1 = mapper.readValue(history, CadFrasePre[].class);
                setHistory(Arrays.asList(mapper.readValue(history, CadFrasePre[].class)));

                Date tempDate = new Date(0);
                User tempUser = null;
                for (CadFrasePre template : pp1) {
