...
@Named
@ViewScoped
public class AddIncentivoPortabilidadeBean implements Serializable {
    ...
    private void initialize() {
        ...
        allDdds = new DualListModel<>(Arrays.asList(TODOS_DDDs), new ArrayList<>());
    ...
    public void addGrupoDdd() {
        if (allDdds != null && allDdds.getTarget() != null) {
            ...
            allDdds.setSource(Arrays.asList(TODOS_DDDs));
        }
    ...
    public void addDdd(SelectEvent evt) {
        if (evt.getObject() != null && allDdds.getSource() != null && allDdds.getSource().contains(String.valueOf(evt.getObject()))) {
            ...
            if (ddds144.get(currentDddGroupIndex).contains(String.valueOf(evt.getObject()))) {
                FacesMessage message = new FacesMessage(FacesMessage.SEVERITY_WARN, "DDD já existe em algum grupo!", null);
                FacesContext.getCurrentInstance().addMessage(null, message);
            ...
            ddds144.get(currentDddGroupIndex).add(String.valueOf(evt.getObject()));
