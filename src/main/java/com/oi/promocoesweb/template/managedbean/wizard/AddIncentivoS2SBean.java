...
import com.oi.promocoesweb.template.managedbean.IoCartaoCartaoS2SBean;
import com.oi.promocoesweb.template.managedbean.IoControleControleS2SBean;
...
@Named
@ViewScoped
public class AddIncentivoS2SBean implements Serializable {
    ...
    public String onFlowProcess(FlowEvent event) {
        ...
        switch (event.getNewStep()) {
            case "tabDados":
                ...
                break;
            case "tabConfirm":
                    if ("IN".equals(tarifador) && "PRE".equals(tipoPlano)) {
                        FacesContext context = FacesContext.getCurrentInstance();
                        Application application = context.getApplication();

                        IoCartaoCartaoS2SBean ioCartaoCartaoS2SBean = application.evaluateExpressionGet(context, "#{ioCartaoCartaoS2SBean}", IoCartaoCartaoS2SBean.class);
                        listaNewTemplateDados = ioCartaoCartaoS2SBean.getNewTemplateList();
                        try {
                        ...
                        IoControleControleS2SBean ioControleControleS2SBean = application.evaluateExpressionGet(context, "#{ioControleControleS2SBean}", IoControleControleS2SBean.class);
                        listaNewTemplateDados = ioControleControleS2SBean.getNewTemplateList();
                        try {
                            for (Template listaNewTemplateDado : listaNewTemplateDados) {
                                listaNewTemplateDado.validate();
                            }
    ...
    public void addDdd(SelectEvent evt) {
        if (evt.getObject() != null && allDdds.getSource() != null && String.valueOf(evt.getObject()) != null) {
            ...
            for (String value : valueOf) {
                if (ddds144.get(currentDddGroupIndex).contains(String.valueOf(value))) {
