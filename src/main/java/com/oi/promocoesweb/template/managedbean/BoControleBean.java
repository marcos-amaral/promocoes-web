...
@Named
@ViewScoped
public class BoControleBean extends TemplateBean<BoControle> implements Serializable {

    private static final Logger logger = LogManager.getLogger();

    private final TabelasEnum tabelasEnum = TabelasEnum.BO_CONTROLE;
    ...
    public void init() {
        ...
        try {
            ...
            super.clearVarsEdit(new BoControle());
            super.clearVarsAdd(new BoControle());
    ...
    public void preLoadEditBean() {
        if (this.getSelectedTemplateList().size() == 1) {
            this.setEditTemplate(new BoControle(this.getSelectedTemplateList().get(0)));
            this.getEditTemplate().setUser(super.getUser());
            this.setNewTemplate(new BoControle(this.getSelectedTemplateList().get(0)));
            this.getNewTemplate().setUser(super.getUser());
        } else {
            this.setEditTemplate(new BoControle());
            this.getEditTemplate().setUser(super.getUser());
            this.setNewTemplate(new BoControle());
            this.getNewTemplate().setUser(super.getUser());
        }
    ...
    @Override
    public List<BoControle> getTemplateList() {
        return (List<BoControle>) (Object) super.getTemplateList();
    }

    @Override
    public List<BoControle> getSelectedTemplateList() {
        return (List<BoControle>) (Object) super.getSelectedTemplateList();
    }

    @Override
    public BoControle getNewTemplate() {
        return (BoControle) super.getNewTemplate();
    }

    @Override
    public BoControle getEditTemplate() {
        return (BoControle) super.getEditTemplate();
    }

    @Override
    public void setTemplateList(List<BoControle> configOfertasPreList) {
        super.setTemplateList(configOfertasPreList);
    }

    @Override
    public void setSelectedTemplateList(List<BoControle> selectedConfigOfertasPreList) {
        super.setSelectedTemplateList(selectedConfigOfertasPreList);
    }

    @Override
    public void setNewTemplate(BoControle newConfigOfertasPre) {
        super.setNewTemplate(newConfigOfertasPre);
    }

    @Override
    public void setEditTemplate(BoControle editConfigOfertasPre) {
        super.setEditTemplate(editConfigOfertasPre);
    }

    public List<BoControle> getSelectedHistoryList() {
        if (super.getSelectedTemplateList() != null && super.getSelectedTemplateList().size() > 0) {
            List<BoControle> history = (List<BoControle>) (Object) super.getSelectedTemplateList().get(0).getHistory();

            for (BoControle bean : history) {
    ...
    public LazyDataModel<BoControle> getDataModel() {
        return (LazyDataModel<BoControle>) (Object) super.getDataModel();
    }
}
